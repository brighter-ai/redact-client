...
def redact_file(file_path: str,
                ...
                region: Region = typer.Option(Region.european_union,
                ...
                custom_labels_file_path: Optional[str] = typer.Option(None, '--labels',
                                                                      help='A JSON file containing custom labels'),
                vehicle_recorded_data: bool = typer.Option(False,
                                                           help='Used to run a job with a variety of iptimizations geared toward vehicle recorded data'),
                speed_optimized: bool = typer.Option(False,
                                                     help='Used to run a job faster with lower accuracy and replacement quality'),
                single_frame_optimized: bool = typer.Option(False,
                                                            help='Used to run a video or archive only as a set of individual images without tracking or interpolation'),
                lp_determination_threshold: float = typer.Option(0.45,
                ...
                face_determination_threshold: float = typer.Option(0.25,
                ...
                licence_plate_custom_stamp_path: Optional[str] = typer.Option(None, '--custom-lp',
                                                                              help='Image file to use for license plate replacements'),
                redact_url: str = typer.Option(None, help='Pass api-key if client is being used with the cloud'),
                api_key: Optional[str] = typer.Option(settings.redact_url_default,
                ...
                skip_existing: bool = typer.Option(False, help='Specify whether to overwrite previously run files'),
                auto_delete_job: bool = typer.Option(False,
                                                     help='Specify whether to automatically delete the job from the backend')
                ):
...
def redact_folder(in_dir: str,
                  ...
                  api_key: Optional[str] = typer.Option(settings.redact_url_default,
                                                        help='Pass api-key if client is being used with the cloud'),
                  n_parallel_jobs: int = typer.Option(1, help='Number of jobs to process in parellel'),
                  save_labels: bool = typer.Option(False, help='Save labels for PII bounding boxes'),
                  skip_existing: bool = typer.Option(False, help='Specify whether to overwrite previously run files'),
                  auto_delete_job: bool = typer.Option(False,
                                                       help='Specify whether to automatically delete the job from the backend')):
