...
from redact_client.data_models import JobArguments, Region, ServiceType, OutputType
from redact_client.settings import Settings
from redact_client.tools.anonymize_file import anonymize_file as anon_file
from redact_client.tools.anonymize_folder import InputType, anonymize_folder as anon_folder
...
def anonymize_file_entry_point():
    """Entry point for redact_anon_file script as defined in 'pyproject.toml'."""
    app = typer.Typer()
    app.command()(anonymize_file)
    app(prog_name='redact_anon_file')
...
def anonymize_folder(in_dir: str, out_dir: str, input_type: InputType, out_type: OutputType, service: ServiceType,
    ...
    anon_folder(in_dir=in_dir, out_dir=out_dir, input_type=input_type, out_type=out_type, service=service,
                ...
                redact_url=redact_url, subscription_key=subscription_key, n_parallel_jobs=n_parallel_jobs,
                save_labels=save_labels, skip_existing=skip_existing, auto_delete_job=auto_delete_job)


def anonymize_folder_entry_point():
    """Entry point for redact_anon_folder script as defined in 'pyproject.toml'."""
    app = typer.Typer()
    app.command()(anonymize_folder)
    app(prog_name='redact_anon_folder')
